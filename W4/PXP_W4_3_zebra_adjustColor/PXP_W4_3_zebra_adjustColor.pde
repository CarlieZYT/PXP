...
void draw() {
  ...
  for (int x = 0; x<width; x++) {
    for (int y = 0; y<height; y++) {
      ...
      int posterizeAmount = max(1, mouseX);                    // posterizeAmount
      posterizeAmount = int(map(posterizeAmount, 1, width, 1, 255));
      
      // condition based on zebra effect
      if ((topPixel.R+topPixel.R+topPixel.B) % posterizeAmount < posterizeAmount/2) {              
        R = bottomPixel.R;
        G = bottomPixel.G;
        B = bottomPixel.B;
      } else {                                                // Adjust color on the top
        R = topPixel.R + mouseX;
        G = topPixel.G + mouseY;
        B = 255 - topPixel.B;
      }
